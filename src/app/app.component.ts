...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    const localData = localStorage.getItem("ProyectoFinal");
    if(localData != null) {
  ...
  onDelete(item: Student) {
    const isDelet = confirm("¿Estás seguro de que quieres eliminarlo?");
    if(isDelet) {
      const currentRecord =  this.studentList.findIndex(m=> m.id === this.studentObj.id);
      this.studentList.splice(currentRecord,1);
      localStorage.setItem('ProyectoFinal', JSON.stringify(this.studentList));
    }
  }
  onEdit(item: Student) {
  ...
  saveStudent() {
    debugger;
    const isLocalPresent = localStorage.getItem("ProyectoFinal");
    if (isLocalPresent != null) {
      ...
      localStorage.setItem('ProyectoFinal', JSON.stringify(oldArray));
    } else {
      const newArr = [];
      newArr.push(this.studentObj);
      this.studentObj.id = 1;
      this.studentList = newArr;
      localStorage.setItem('ProyectoFinal', JSON.stringify(newArr));
    }
    this.closeModel()
  }
}
